<?php
	...
	$total_record = $lumise->db->rawQuery("SELECT count(*) as total FROM {$lumise->db->prefix}cliparts");
	...
	$config = array(
		...
		'total_page'    => ceil($total_record[0]['total']/$per_page),
 	    'limit'         => $per_page,
	    'link_full'     => $lumise_router->getURI().'lumise-page=cliparts&tpage={page}',
	    'link_first'    => $lumise_router->getURI().'lumise-page=cliparts',
	);
						...
						foreach ($arts as $art) { ?>
							...
							<tr>
								...
								<td class="lumise-resource-title">
									<a href="<?php echo $lumise_router->getURI();?>lumise-page=clipart&id=<?php if(isset($art['id'])) echo $art['id'] ?>" class="name"><?php if(isset($art['name'])) echo $art['name']; ?></a>
