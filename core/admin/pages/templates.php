<?php
	...
	$total_record = $lumise->db->rawQuery("SELECT count(*) as total FROM {$lumise->db->prefix}templates");
    ...
    $config = array(
		...
		'total_page'    => ceil($total_record[0]['total']/$per_page),
 	    'limit'         => $per_page,
	    'link_full'     => $lumise_router->getURI().'lumise-page=templates&tpage={page}',
	    'link_first'    => $lumise_router->getURI().'lumise-page=templates',
	);
								...
								foreach ($templates as $value) { ?>
									...
									<tr>
										...
										<td class="lumise-resource-title">
											<a href="<?php echo $lumise_router->getURI();?>lumise-page=template&id=<?php if(isset($value['id'])) echo $value['id']; ?>" class="name"><?php if(isset($value['name'])) echo $value['name']; ?></a>
