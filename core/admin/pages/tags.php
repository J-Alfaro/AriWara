<?php
    ...
    $config = array(
		...
		'total_page'    => $tags['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $lumise_router->getURI().'lumise-page=tags&type='.$type.'&tpage={page}',
	    'link_first'    => $lumise_router->getURI().'lumise-page=tags&type='.$type,
	);
					...
					<tbody>
						<?php foreach ($tags['rows'] as $value) {?>
								<tr>
									...
									<td><a href="<?php echo $lumise_router->getURI();?>lumise-page=tag&type=<?php echo $type; ?>&id=<?php if(isset($value['id'])) echo $value['id']; ?>" class="name"><?php if(isset($value['name'])) echo $value['name']; ?></a></td>
								</tr>
						<?php } ?>
					</tbody>
				</table>
